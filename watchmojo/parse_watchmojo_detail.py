...
for video in parsing_videos:
	...
	try:
		...
		# print(e1)
		try:
			vid_youtube = driver.find_element_by_id('videoplay').find_element_by_tag_name('iframe')
			video_link = vid_youtube.get_attribute('src')
			vid_type = 'youtube'
		except Exception as e2:
			try:
				vid_youtube = driver.find_element_by_id('mainplayer').find_element_by_tag_name('iframe')
				video_link = vid_youtube.get_attribute('src')
				vid_type = 'youtube'
				...
				try:
					...
					try:
						vid_normal = driver.find_element_by_id('videoplay').find_element_by_xpath('//video/source')
						video_link = vid_youtube.get_attribute('src')
						vid_type = 'normal'
